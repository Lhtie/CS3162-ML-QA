...
class QAModel_DESC(nn.Module):
    def __init__(self, max_len=512):
        super(QAModel_DESC, self).__init__()
    ...
    def train(self, epochs=3, batch_size=48, lr=3e-5, eps=1e-6, full=False):
        if not full:
            train_inputs, train_labels = data_process.QA_DataProcessor().retrieve_train_data()
        else:
            train_inputs, train_labels = data_process.QA_DataProcessor().retrieve_full_train_data("../train-v2.0.json")
    ...
    def prediction(self, res_file, batch_size=48, tau=1.0):
        eval_inputs, eval_labels = data_process.QA_DataProcessor().retrieve_eval_data()
