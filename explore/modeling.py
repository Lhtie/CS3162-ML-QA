...
class QuestionAnswering(nn.Module):
    def __init__(self, qc_max_len=128, qa_max_len=512, threshold=0.8, tau=0.01):
    ...
    def forward(self, questions, contexts): # list of batch_size
        ...
        is_impossible, inputs = [], []
        for idx in range(batch_size):
            tokens = self.XLNetTokenizer(answers[idx] + "<sep>" + questions[idx], contexts[idx], return_tensors="pt", 
                            padding="max_length", truncation=True, max_length=self.qa_max_len)
            input_ids = tokens.input_ids.reshape(-1)
            token_type_ids = tokens.token_type_ids.reshape(-1)
            attention_mask = tokens.attention_mask.reshape(-1)
            index = (token_type_ids == torch.zeros(1)).nonzero().flatten()
            for i in index.tolist():
                token_type_ids[i] = 4 # qas: 0, ctx: 1, <cls>: 2, <pad>: 3
                if input_ids[i] == 4: # id of <sep>: 4
                    break
            inputs.append(torch.stack([input_ids, token_type_ids, attention_mask]))

        inputs = torch.stack(inputs).to(device)
        verify = self.verifier(inputs)
        for idx in range(batch_size):
            s_null = start_logits[idx, -1] + end_logits[idx, -1]
            if torch.max(start_logits[idx]) + torch.max(end_logits[idx]) <= s_null + self.tau\
                or torch.sigmoid(verify[idx]) > self.threshold:
