...
class QuestionAnsweringModel(nn.Module):
    def __init__(self, max_len=512, tau=1.0):
    ...
    def train(self, train_file, epochs=3, batch_size=48, lr=3e-5, eps=1e-6):
        train_inputs, train_labels = data_process.DataProcessor(trainFile=train_file).retrieve_train_data()
    ...
    def retrieve_answer(self, input, start_logits, end_logits): # input: InputType, output: ModelOutputs
        s_null = start_logits[-1] + end_logits[-1]
        if (torch.max(start_logits) + torch.max(end_logits) <= s_null +  self.tau):
            return ""
    ...
    def prediction(self, eval_file, res_file, batch_size=48):
        eval_inputs = data_process.DataProcessor(evalFile=eval_file).retrieve_eval_data()
            ...
            for batch_start in range(0, len(inputs), batch_size):
                ...
                for idx in range(batch_start, batch_end):
                    answers[eval_inputs[idx]["id"]] = self.retrieve_answer(inputs[idx], 
                            outputs.start_logits[idx-batch_start], outputs.end_logits[idx-batch_start])
        ...
        data_process.DataProcessor(resFile=res_file).write_result(answers)
